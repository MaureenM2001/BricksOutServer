...
namespace GameServer
{
    ...
    public class Packet : IDisposable
    {
        ...
        /*
        public int ReadInt1(bool _moveReadPos = true)
        {
            if (buffer.Count > readPos)
            {
                int[] _value = new int[Constants.BrickConst];
                // If there are unread bytes
                ...
                for (int i = 0; i < Constants.BrickConst; i++)
                {
                    _value[i] = BitConverter.ToInt32(readableBuffer, readPos); // Convert the bytes to an int
                    if (_moveReadPos)
                    {
                        // If _moveReadPos is true
                        readPos += 4; // Increase readPos by 4
        ...
        public int Readint1(bool _moveReadPos = true)
        {
            ...
            {
                // If there are unread bytes
                int[] _value = new int[BrickConst];
                for (int i = 0; i < BrickConst; i++)
                {
                    _value[i] = BitConverter.Toint32(readableBuffer, readPos); // Convert the bytes to an int
                    if (_moveReadPos)
                    {
        ...
        */
        ...
        /// <summary>Reads a bool[,] from the packet.</summary>
        /// <param name="_moveReadPos">Whether or not to move the buffer's read position.</param>
        public bool ReadBool2(int row, int column, bool _moveReadPos)
        {
            if (buffer.Count > readPos)
            {
                bool[,] _value = new bool[row,column];
                // If there are unread bytes
                for(int i = 0; i < row; i++){
                    for(int j = 0; j < column; j++){
                        _value[i,j] = BitConverter.ToBoolean(readableBuffer, readPos); // Convert the bytes to a bool
                        if (_moveReadPos)
                        {
                            // If _moveReadPos is true
                            readPos += 1; // Increase readPos by 1
